...
def new_ticket():
    ...
    if form.validate_on_submit():
        ...
        # Build from department, subunit and location fields present in the form
        parts = []
        if getattr(form, 'location_department', None) and form.location_department.data:
            parts.append(form.location_department.data)
        if getattr(form, 'location_subunit', None) and form.location_subunit.data:
            parts.append(form.location_subunit.data)
        if getattr(form, 'location', None) and form.location.data:
            parts.append(form.location.data)
        # Fallback to description of location or empty string
        location = ' - '.join([p for p in parts if p]) if parts else (form.location.data if getattr(form, 'location', None) else '')
        ...
        # Optional MIS subcategory field may not exist on the form; check before using
        if category_obj and category_obj.name == 'University MIS System Issue' and getattr(form, 'mis_subcategory', None) and form.mis_subcategory.data:
